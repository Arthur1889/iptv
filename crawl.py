...
def clean_channel_name(name):
    """规则6增强版：更激进地裁剪画质后缀、特殊符号、空格，专门消灭综合频道重复"""
    if not name: return ""
    # 移除各种常见画质、地区、蓝光、超清等狗皮膏药后缀，全部转大写处理
    name = name.upper()
    patterns = [
        r'360P', r'404P', r'480P', r'576P', r'606P', r'720P', r'1080P', r'2160P', r'4K', r'8K',
        r'HD', r'FHD', r'UHD', r'SD', r'HEVC', r'H264', r'H265', r'蓝光', r'超清', r'高清', r'标清',
        r'NOT\s*24/7', r'GEO-BLOCKED', r'\[.*\]', r'\(.*\)', r'（.*）', r'【.*】', 
        r'-PLUS', r'\+', r'频道', r'🔤', r'🇨🇳', r'▶️'
    ]
    for p in patterns:
        name = re.sub(p, '', name)
    
    # 彻底移除所有空格、减号、下划线，让 "CHC 动作电影" 和 "CHC动作电影HD" 变成同一个 key
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5, context=ctx) as response:
            ...
            if status == 200 or status == 302:
                ...
                if '4K' in name_upper or '2160' in name_upper or 'UHD' in name_upper or '超高清' in name_upper: return 2160
                if '1080' in name_upper or 'FHD' in name_upper or '蓝光' in name_upper or '超清' in name_upper: return 1080
                if '720' in name_upper or 'HD' in name_upper or '高清' in name_upper: return 720
...
def print_progress(processed, total, start_time, valid_count):
    ...
    sys.stdout.write(f"\r🚀 探测进度: [{bar}] {percent:.1f}% | 速度:{speed:.1f}源/s | 优质源:{valid_count}/{total}    ")
    sys.stdout.flush()
...
def main():
    ...
    for i in range(len(lines)):
        if lines[i].startswith("#EXTINF"):
            ...
            # 强力去重清洗：基于激进清洗的名字去匹配 name.json 
            clean_raw = clean_channel_name(display_name)
            std_name = clean_raw
            
            for key, aliases in name_db.items():
                cleaned_aliases = [clean_channel_name(a) for a in aliases]
                if clean_channel_name(tvg_id) in cleaned_aliases or clean_channel_name(tvg_name) in cleaned_aliases or clean_raw in cleaned_aliases:
                    std_name = key
                    break
    ...
    for name, cats in grouped_channels.items():
        ...
        # 🚨 修复核心：只要探测出来是 4K/8K 盲猜源，无条件强制归入“4K频道”组！
        if best_4k:
            best_4k['group'] = "4K频道"
