...
def fetch_and_process():
    ...
    print(f">>> 深度分析择优中 (总数: {len(all_channels)})...")
    best_channels = {}
    with ThreadPoolExecutor(max_workers=10) as executor:
        ...
        for f in as_completed(futures):
            ...
            if is_ok:
                name = res_ch['name']
                if name not in best_channels or (res_ch['height'] > best_channels[name]['height']) or \
...
def save_m3u(channels):
    """为 Apple TV APTV 优化的导出格式"""
    with open("tv.m3u", "w", encoding="utf-8") as f:
        f.write(f'#EXTM3U x-tvg-url="{EPG_URL}"\n')
        for ch in channels:
            # 自动生成 APTV 需要的台标和节目单 ID
